...
const consultarFeriados = async (ano, mes, pais) => {
  const url = `https://calendarific.com/api/v2/holidays?&api_key=${calendarificKey}&country=${pais}&year=${ano}&type=national&language=pt`;
  try {
    const res = await fetch(url);
...
btn.addEventListener("click", async () => {
  ...
  const feriados = await consultarFeriados(ano, mes, local.country.toUpperCase());
