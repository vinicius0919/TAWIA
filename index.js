...
const consultarFeriados = async (ano, mes, pais) => {
  const url = `https://calendarific.com/api/v2/holidays?&api_key=${calendarificKey}&country=${pais}&month=${mes}&type=local&language=pt`;
  try {
    const res = await fetch(url);
...
btn.addEventListener("click", async () => {
  ...
  const feriados = await consultarFeriados(mes, local.country.toUpperCase());
